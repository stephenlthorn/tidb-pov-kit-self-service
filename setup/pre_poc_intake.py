...
def tier_test_profile(tier: str) -> Dict:
    """Return tier-specific test profile defaults.

    All sizes use the same ~3GB S3 dataset. load_size controls concurrency,
    duration, and QPS intensity — not data volume.
    """
    profiles = {
        "serverless": {
            "load_size": "small",
        ...
        "premium": {
            ...
            "warm_phase_concurrency": 128,
            "point_get_phase_enabled": True,
            "point_get_duration_seconds": 180,
            "point_get_concurrency": 128,
            "import_batch_size": 10000,
            "import_into_threads": 4,
        },
        "dedicated": {
            "load_size": "large",
            "duration_seconds": 300,
...
# ─── Load intensity profiles (keyed by load_size, not data volume) ───────────
